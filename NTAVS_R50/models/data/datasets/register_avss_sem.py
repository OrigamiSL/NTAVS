...
def _get_avss_files(df_split, root, split, pre_mask_dir=None,
    ...
    for index in np.arange(0, len(df_split)):
        # iloc: output the 'index' row of the dataframe
        ...
        audio2_file = os.path.join(root, set, video_name, "audio_256_96_new_scale.pkl")
        audio3_file = os.path.join(root, set, video_name, "audio_1024_96_new_scale.pkl")
...
def load_avss_semantic(df_split, root, split, pre_mask_dir):
    ...
    for image_file, label_file, audio_file, pre_mask_file, vid_temporal_mask_flag, gt_temporal_mask_flag,\
...
if _root.endswith("/AVSBench_semantic/"):
    print('_root', _root)
    _root = './AVS_dataset/AVSBench_semantic'
