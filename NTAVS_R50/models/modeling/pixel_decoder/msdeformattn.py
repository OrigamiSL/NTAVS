...
class MSDeformAttnPixelDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.linear1 = nn.Conv2d(conv_dim*16, conv_dim, kernel_size=1)
        ...
        for idx, in_channels in enumerate(self.feature_channels[: self.num_fpn_levels]):
            ...
            lateral_conv = Conv2d(256, conv_dim, kernel_size=1, bias=use_bias, norm=lateral_norm)
            # in_channel: always 256
            output_conv = Conv2d(
                conv_dim,
                conv_dim,
                kernel_size=3,
    ...
    def forward_features(self, features):
        #print('f', self.feature_channels[: self.num_fpn_levels])
        srcs = []
        pos = []
        ...
        # y1 = cur_fpn.contiguous().view(b,c,h//4,4,w//4,4)
        # y1 = y1.permute(0,1,3,5,2,4).contiguous().view(b,c*16,h//4,w//4)
        # y2 =self.linear1(y1)
        # y  = mid_feature + y2
        # y = output_conv(y)
        # # print(y.shape)
        ...
        for o in out:
            if num_cur_levels < self.maskformer_num_feature_levels:
                ...
                num_cur_levels += 1
        # print('1', self.mask_features(out[-1]).shape)
