...
def frame(data, window_length, hop_length): # # 512/400, 160
...
def spectrogram_to_mel_matrix(
    ...
    """
    nyquist_hertz = audio_sample_rate / 2.0 # 8000
    ...
    spectrogram_bins_hertz = np.linspace(0.0, nyquist_hertz, num_spectrogram_bins) #0,8000, 257
...
def log_mel_spectrogram(data, audio_sample_rate=8000, log_offset=0.0, window_length_secs=0.025, hop_length_secs=0.010,
                        window_length_new = 512, 
                        **kwargs):
    ...
    window_length_samples = window_length_new #512 #400
    ...
    print('spectrogram', spectrogram.shape) # (500, 257) (499, 257)
    xx = spectrogram_to_mel_matrix(num_spectrogram_bins=spectrogram.shape[1], audio_sample_rate=audio_sample_rate, **kwargs)
    print(xx.shape) #257, n_mels
    mel_spectrogram = np.dot(
        spectrogram, spectrogram_to_mel_matrix(num_spectrogram_bins=spectrogram.shape[1], audio_sample_rate=audio_sample_rate, **kwargs)
    ) 
    # 500 * n_mels
...
def stft_spectrogram(data, audio_sample_rate=8000, log_offset=0.0, window_length_secs=0.025, hop_length_secs=0.010,
                        window_length_new = 512, 
                        **kwargs):
