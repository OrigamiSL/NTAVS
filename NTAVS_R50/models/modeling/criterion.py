...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, num_points, oversample_ratio, importance_sample_ratio):
        ...
        self.indices_first_frame = None  # * Add for avss4
    ...
    def loss_labels(self, outputs, targets, indices, num_masks):
        ...
        # torch.Size([8, 96, 3]) torch.Size([8, 96]) torch.Size([3])
        # print(src_logits.shape, target_classes.shape, self.empty_weight.shape)
    ...
    def forward(self, outputs, targets):
        ...
        if len(outputs[output_keys[0]]) != len(targets):  # * Add for avss4  
            if self.indices_first_frame is None:
                self.indices_first_frame = torch.tensor(list(range(0, len(outputs[output_keys[0]]), 5)))
                self.indices_first_frame = self.indices_first_frame.cuda()

            for output_key in output_keys:
                # * only select the first frame's mask
                    ...
                    for aux_out_layers in range(len(outputs[output_key])):
                        for aux_keys in outputs[output_key][aux_out_layers].keys():
                            outputs[output_key][aux_out_layers][aux_keys] = torch.index_select(
                                outputs[output_key][aux_out_layers][aux_keys], dim=0, index=self.indices_first_frame
                            )
                else:
                    outputs[output_key] = torch.index_select(outputs[output_key], dim=0, index=self.indices_first_frame)
        ...
        # cosine_weight = outputs['similarity_images']
        cosine_weight = None
