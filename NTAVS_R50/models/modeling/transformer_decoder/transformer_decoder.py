...
class MultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        queries_fuse_type: str,  # * add for queries fuse
        ...
        # * query type
        self.queries_fuse_type = queries_fuse_type
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret["queries_fuse_type"] = cfg.MODEL.FUSE_CONFIG.QUERIES_FUSE_TYPE
    ...
    def forward(self, x, audio_features, mask_features, new_fused_feature = None, mask=None):
        ...
        elif self.queries_fuse_type == "dim":
            output = torch.cat([output, audio_features], dim=-1)
        elif self.queries_fuse_type == "all":
            output = audio_features
        elif self.queries_fuse_type == "none":
            pass

        NTAVS_query_feat = output
    ...
    def forward_prediction_heads(self, output, mask_features, attn_mask_target_size):
        ...
        thre = 0.2
        attn_mask = (attn_mask.sigmoid().flatten(2).unsqueeze(1).repeat(1, self.num_heads, 1, 1).flatten(0, 1) < thre).bool()
