...
from models.modeling.audio_backbone.CED.CED_process_audio import get_CED_process
...
def _get_avss_files(df_split, root, split, f_size, t_size):
    files = []
    basic_root = os.path.join(root, 'ms3_data/audio_wav'+'_'+str(f_size)+'_'+str(t_size)+'_'+'new_scale')
    os.makedirs(basic_root, exist_ok= True)
    for index in tqdm(np.arange(0,len(df_split))):
        df_one_video = df_split.iloc[index]
        video_name, set = df_one_video['video_id'], df_one_video['split']
        os.makedirs(os.path.join(basic_root, set), exist_ok= True)
       
        audio_path = os.path.join(root, 'ms3_data/audio_wav', set, video_name+'.wav')
        audio_sacle_path = os.path.join(basic_root, set, video_name+'.pkl')
        ...
        # x = get_CED_process(audio_path, mel_bins = f_size)
        # print('x', x.shape)
        ...
        #     new_x = torch.zeros((10,1 ,t_size, f_size))
        #     new_x[:x.shape[0]] = x
        # else:
        #     new_x = x
...
if __name__=="__main__":
    ...
    img_dir = './AVS_dataset/AVSBench_object/Multi-sources'
    splits = ["train","val","test"]
    df_all = pd.read_csv(os.path.join(img_dir,'ms3_meta_data.csv'), sep=',')
