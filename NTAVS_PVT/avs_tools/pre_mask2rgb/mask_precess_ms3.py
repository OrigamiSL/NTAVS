...
if __name__ == "__main__":
    ...
    gt_dir = args.gt_dir + '/AVSBench_object/Multi-sources/ms3_data/visual_frames/' 
    save_dir = args.save_dir + '/AVSBench_object/Multi-sources/ms3_data/pre_SAM_mask/'
    videos = PathManager.ls(gt_dir)
    videos = sorted(videos)
    print(f"{len(videos)} videos found in '{gt_dir}'.")
    max_idx = 0

    for video in tqdm(videos):
        video_gt_dir = os.path.join(gt_dir, video)
        save_video_gt_dir = os.path.join(save_dir, video)
