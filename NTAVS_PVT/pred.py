...
class Trainer(DefaultTrainer):
    ...
    @classmethod
    def test(cls, cfg, model, evaluators=None, if_train = True):
        """
        ...
        for idx, dataset_name in enumerate(cfg.DATASETS.TEST):
            ...
            if dataset_name[:5] == 'avss_':
                results_i = inference_on_dataset_ss(model, data_loader, evaluator, cfg.OUTPUT_DIR, dataset_name, if_train)
            else:
                results_i = inference_on_dataset(model, data_loader, evaluator, cfg.OUTPUT_DIR, dataset_name, if_train)
...
def setup(args):
    ...
    setup_logger(output=output_dir, distributed_rank=comm.get_rank(), name="NTAVS")
    return cfg
...
def main(args):
    ...
    if args.eval_only:
        ...
        res = Trainer.test(cfg, model, if_train= False)
        if comm.is_main_process():
