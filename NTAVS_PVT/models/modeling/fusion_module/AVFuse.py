...
from .utils.fuse_helper import BiAttentionBlock, BiAttentionBlock_L, OneStreamAttentionBlock,\
AttentionA2I, OneStreamBlock
...
class AVFuse(nn.Module):
    ...
    def __init__(self, fused_type, audio_dim, fused_backbone, fused_backbone_dim):
        ...
        self.audio_pos = nn.Embedding(144, self.audio_dim)
        ...
        if self.fused_type == "MHA-Patch-Audio":
            self.patch_attn = OneStreamBlock(
                visual_features_names=self.fused_backbone,
                vision_dim_list=self.fused_backbone_dim,
                audio_dim=self.audio_dim,
                embed_dim=self.embed_dim,
    ...
    def __call__(self, visual_features, audio_features):
        ...
        for i in range(len(visual_features)):
            ...
            # add level_emb for different size
