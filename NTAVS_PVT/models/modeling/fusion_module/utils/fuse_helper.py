...
class BiMultiHeadAttention_L(nn.Module):
    def __init__(self, v_dim, a_dim, embed_dim, num_heads, dropout=0.1):
        """
        v_dim: visual feature dimension
        a_dim: audio feature dimension
        ...
        num_heads: number of heads
        """
        super(BiMultiHeadAttention_L, self).__init__()
        ...
        self.vq_proj = nn.Linear(self.v_dim, self.embed_dim)
        self.aq_proj = nn.Linear(self.a_dim, self.embed_dim)

        self.vk_proj = nn.Linear(self.v_dim, self.embed_dim)
        self.ak_proj = nn.Linear(self.a_dim, self.embed_dim)
        ...
        self.norm_v_1 = nn.LayerNorm(self.embed_dim)
        self.norm_v_2 = nn.LayerNorm(self.embed_dim)
        self.norm_a_1 = nn.LayerNorm(self.embed_dim)
        self.norm_a_2 = nn.LayerNorm(self.embed_dim)

        self.v_mlp = Mlp(self.embed_dim, self.embed_dim * 4)
        self.a_mlp = Mlp(self.embed_dim, self.embed_dim * 4)
    ...
    def _reset_parameters(self):
        nn.init.xavier_uniform_(self.vq_proj.weight)
        self.vq_proj.bias.data.fill_(0)
        nn.init.xavier_uniform_(self.aq_proj.weight)
        self.aq_proj.bias.data.fill_(0)
        nn.init.xavier_uniform_(self.vk_proj.weight)
        self.vk_proj.bias.data.fill_(0)
        nn.init.xavier_uniform_(self.ak_proj.weight)
        self.ak_proj.bias.data.fill_(0)
        nn.init.xavier_uniform_(self.values_v_proj.weight)
        self.values_v_proj.bias.data.fill_(0)
        nn.init.xavier_uniform_(self.values_a_proj.weight)
        self.values_a_proj.bias.data.fill_(0)
    ...
    def forward(self, v, a, pos_v=None, pos_a=None):
        ...
        q_visual = (self.vq_proj(v + pos_v)) * self.scale
        k_visual = (self.vk_proj(v + pos_v)) * self.scale

        q_audio = self._shape(self.aq_proj(a), -1, bsz)
        k_audio = self._shape(self.ak_proj(a), -1, bsz)
        ...
        q_visual = self._shape(q_visual, tgt_len, bsz).view(*proj_shape)  
        k_visual = self._shape(k_visual, tgt_len, bsz).view(*proj_shape)

        q_audio = self._shape(q_audio, audio_len, bsz).view(*proj_shape)  
        k_audio = self._shape(k_audio, audio_len, bsz).view(*proj_shape)

        value_v_states = value_v_states.view(*proj_shape)  # * [bs*5*8, 56*56, 256] vision feature = value
        value_a_states = value_a_states.view(*proj_shape)  # * [bs*5*8, 1, 256]  audio feature = value

        attn_weights_v_a = torch.bmm(q_visual, k_audio.transpose(1, 2).contiguous()) 
        attn_weights_v_a = attn_weights_v_a.softmax(dim=-1) 

        attn_weights_a_v = torch.bmm(q_audio, k_visual.transpose(1, 2).contiguous()) 
        attn_weights_a_v = attn_weights_a_v.softmax(dim=-1)

        attn_probs_v_a = F.dropout(attn_weights_v_a, p=self.dropout, training=self.training)
        attn_probs_a_v = F.dropout(attn_weights_a_v, p=self.dropout, training=self.training)

        attn_output_v = torch.bmm(
            attn_probs_v_a, value_a_states
        )  
        attn_output_a = torch.bmm(
            attn_probs_a_v, value_v_states
        )  
        ...
        attn_output_a = attn_output_a.view(bsz, self.num_heads, audio_len, self.head_dim)
        attn_output_a = attn_output_a.transpose(1, 2).contiguous()
        attn_output_a = attn_output_a.reshape(bsz, audio_len, self.embed_dim)  # * [bs*5, 1, 2048]

        attn_output_v = self.out_v_proj(attn_output_v)  # * Linear(2048,256)  -> [bs*5, 56*56, 256]
        attn_output_a = self.out_a_proj(attn_output_a)  # * Linear(2048,128)  -> [bs*5, 1, 128]

        v = v + attn_output_v
        out_v = v + self.v_mlp(self.norm_v_1(v))
        out_v = self.norm_v_2(out_v)

        a = a + attn_output_a
        out_a = a + self.a_mlp(self.norm_a_1(a))
        out_a = self.norm_a_2(out_a)

        # print('attn_output_v, attn_output_a', attn_output_v.shape, attn_output_a.shape)
...
class OneStreamBlock(nn.Module):
    def __init__(
        self,
        visual_features_names,
        vision_dim_list,
        ...
        super(OneStreamBlock, self).__init__()
    ...
    def forward(self, visual_features, audio_feature, pos_a=None, pos_v=None):
        ...
        for num_level, feature_name in enumerate(self.visual_features_names):
            ...
            # * start fusion
            new_v, new_a = \
                self.one_stream_attention(v=visual_feature_flatten, a=audio_feature, level=num_level, pos_a=pos_a, pos_v=pos_v)
                # self.single_attention_call(v=visual_feature_flatten, a=audio_feature, level=num_level, pos_a=pos_a, pos_v=pos_v)
            # * [bs*5, 56*56, 256] [bs*5, 128]
            # [bs, N, C] -> [bs, C, N]
            new_v = new_v.transpose(1, 2).contiguous()
            new_v_list.append(new_v)
...
class BiAttentionBlock_L(nn.Module):
    def __init__(
        self,
        visual_features_names,
        vision_dim_list,
        ...
        Inputs:
            ...
            dropout - Amount of dropout to apply in the feed-forward network
        """
        super(BiAttentionBlock_L, self).__init__()
